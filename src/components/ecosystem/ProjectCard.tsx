...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  const handleVote = async (vote: "up" | "down") => {
    ...
    const previousBreakdown = project.votes?.breakdown
      ? { ...project.votes.breakdown }
      : undefined;

    try {
      setIsVoting(true);

      // Calculate vote changes for optimistic update
      let upvoteChange = 0;
      ...
      if (project.votes?.breakdown && user.canVote) {
        const userRole = ROLE_TIERS.find(
          (role) => user.id && project.votes?.breakdown[role.name]
        )?.name;

        if (userRole && project.votes.breakdown[userRole]) {
          // Remove previous vote if exists
          if (previousUserVote) {
            project.votes.breakdown[userRole][previousUserVote]--;
          }
          // Add new vote if not removing
          if (previousUserVote !== vote) {
            project.votes.breakdown[userRole][vote]++;
          }
        }
      }
      ...
      // Update project votes in parent component
      if (project.votes) {
      ...
      // Fetch latest votes silently in background
      fetch("/api/votes")
        .then((res) => res.json())
        .then((votesData) => {
  ...
  const getVoteBreakdownText = (voteType: "up" | "down") => {
    if (!project.votes?.breakdown) return "";

    return ROLE_TIERS.map((role) => {
      const roleVotes = project.votes!.breakdown[role.name] || {
        up: 0,
        down: 0,
      };
                  ...
                  <Tooltip.Trigger asChild>
                    <button
                      ...
                      className={cn(
                        ...
                        !user?.canVote && "opacity-50 cursor-not-allowed",
                        userVote === "up"
